...
def getProbDist(go_text=.33, stop_text=.33, ai_text=.34, ai_stop = .5, n_text=15):
    ...
    prob_dist.addText('Enter the probability of the ai trials stopping for you.')
    prob_dist.addText('Trial types should add to 1.0 and nTrials should be divisible by 3')
    ...
    prob_dist.addField('AI Stop Probability: ', ai_stop)
    prob_dist.show()
    if prob_dist.OK:
    ...
    # obtains a list that dictates if the ai will stop for you or not
    ai_stopTrials = np.random.choice([0, 1], size=aiTrials, p=[1-ai_stop, ai_stop])

    return conditions, ai_stopTrials
...
if __name__ == "__main__":
    ...
    # Testing the conditions stuff
    conditions, ai_stopTrials = getProbDist()
    file = open("shuffling_test.txt", 'w+')
    content = str(conditions)
    file.write(content)
    file.close()
    ...
    for i in stims:
        if i == 'ai':
            stimlist.append({'condition': i, 'ai_stop': ai_stopTrials[count]})
            count+=1
        else:
            stimlist.append({'condition': i})
    ...
    for trial in trials:
        ...
        # Init Trial Data
        SSD = sample_SSD()
        ai_control, ai_ssd = sample_ai_ssd()
        print(ai_control, ai_ssd)
        print(ai_ssd - ai_control)
        trials.data.add('SSD', SSD)
        trials.data.add('AI Trial SSD', ai_ssd)
        trials.data.add('AI delay', ai_control)
        waitingForSpace = True
        Hit = False
        FinishLine = False
        stillMoving = True
        ...
        if trial['condition'] == 'stop':
            ...
            while feedback_timer.getTime() > 0:
                ...
                finishline.draw()
                mywin.flip()
            trial_end = core.getTime()
            print("trial_time: ", trial_end)
            print("time elapsed", trial_end - trial_start)

        elif trial['condition'] == 'ai':
            trial_start = core.getTime()
            # SSD
            SSD_timer = core.CountdownTimer(ai_ssd - ai_control)
            ai_timer = core.CountdownTimer(ai_control)
            while (ai_timer.getTime() > 0):
                scan_codes, analog_codes, _ = wp.read_full_buffer()
                ring.pos += RING_PACE
                ball.pos += (RING_PACE[0] + [-.031, .035][random.randrange(2)], RING_PACE[1])
                if analog_codes:
                    pressures.append(np.max(analog_codes))
                else:
                    pressures.append(0)
