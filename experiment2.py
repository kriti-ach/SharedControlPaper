...
COUNTDOWN_TIME = 2
STOP_SIGNAL_TIME = 1.5
FEEDBACK_TIME = 2 # just in the practice block
TRIAL_TIME = 6.5 # the total trial time excluding the countdown time
BREAK_TIME = 30.0
BLOCK_END_TIME = 30.0
...
HIT_FEEDBACK = (
    "Please try to keep the dot within the circle as it moves across the screen."
)
...
if __name__ == "__main__":
    ...
    trials.data.add("condition_order", condition_order)
    ...
    ) # this is only used in the practice block
    ...
    break_feedback = visual.TextStim(
        win=mywin,
        text="Take a 10 second break and review your feedback if there is any. The task will resume shortly...",
        height=1,
        pos=[0, 3.5],
        wrapWidth=30,
    )
    ...
    AI_block_text = visual.TextStim(
        win=mywin, text=AI_TEXT, height=1, pos=[0, 0], wrapWidth=30
    )

    Non_AI_block_text = visual.TextStim(
        win=mywin, text=NON_AI_TEXT, height=1, pos=[0, 0], wrapWidth=30
    )
    ...
    # StopSignal
    end_trial_timer = core.CountdownTimer(STOP_SIGNAL_TIME)
    ...
    grey_screen_timer = core.CountdownTimer(TRIAL_TIME - (4 + STOP_SIGNAL_TIME))
    while end_trial_timer.getTime() > 0:
        mywin.color = "grey"
        ball.draw()
        ring.draw()
    ...
    for count, trial in enumerate(trials):
        ...
        if trial["condition"] == "stop":
            ...
            grey_screen_timer = core.CountdownTimer(TRIAL_TIME - (SSD + STOP_SIGNAL_TIME))
            while grey_screen_timer.getTime() > 0:
                mywin.color = "grey"
                ball.draw()
                ring.draw()
                finishline.draw()
        ...
        if count + 1 == practice_len + block:
            ...
            while block_end_timer.getTime() > 0:
                mywin.color = "grey"
                block_feedback.draw()
                if late_proportions >= 0.25:
                    practice_feedback_late.draw()

                if pressure_proportions >= 0.25:
                    practice_feedback_pressure.draw()

                if hit_proportions >= 0.25:
                    practice_feedback_hit.draw()

                if no_depress_proportions >= 0.25:
    ...
    no_depress_proportions = feedback_tracker.count("no_depress") / (
        practice_len + block
    )
